...
`,c.showModal(),a?.focus()}),r?.addEventListener(`click`,()=>{l.showModal()}),i?.addEventListener(`click`,()=>{l.close()}),n?.addEventListener(`click`,()=>{c.close()})}document.getElementById(`chooseMinigame`)?.classList.toggle(`hide`),e();